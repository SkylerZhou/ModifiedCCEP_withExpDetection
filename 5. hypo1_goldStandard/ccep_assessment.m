function ccep_assessment(out, num_subplots, pt_id, sz_id, ccep_overlap_stims, ccep_overlap_resps, ccep_only_stims, ccep_only_resps)
...
num_overlap = numel(ccep_overlap_stims); 
num_only    = numel(ccep_only_stims);    
...
for i = 1:num_subplots        
    ...
    if overlap_idx <= num_overlap
          
        % --- Plot for a CCEP Overlap Pair ---    
        % get the row and col idx for out 
        stim_ch = ccep_overlap_stims{overlap_idx};
        resp_ch = ccep_overlap_resps{overlap_idx};
        ...
        % --- Plot for a CCEP-Only Pair ---
        % get the row and col idx for out 
        stim_ch = ccep_only_stims{only_idx};
        resp_ch = ccep_only_resps{only_idx};
        row = find(strcmp(out.bipolar_labels, stim_ch));
        col = find(strcmp(out.bipolar_labels, resp_ch));
...
set(fig, 'Units', 'normalized', 'Position', [0.05 0.05 1.0 0.8]);
